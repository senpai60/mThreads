...
function LoginSignup() {
  ...
  const onSubmit = (e) => {
    ...
    if (isLogin) {
      ...
      console.log('Registration attempt with:', { username, email, password, fullname });
      // TODO: Implement API call to /mthreads/users/register
    }
  };

  const commonInputClass = "w-full p-3 my-2 rounded-lg bg-zinc-800/80 border border-zinc-700 text-zinc-200 placeholder-zinc-500 focus:outline-none focus:ring-1 focus:ring-zinc-500";
  const buttonClass = "w-full p-3 mt-4 rounded-lg font-semibold transition-colors duration-200";
  ...
  return (
    <div className="w-full h-dvh bg-zinc-950 flex flex-col justify-center items-center text-zinc-200">
      ...
      <div className="text-4xl font-bold mb-8 flex items-center gap-2">
        Thre<FaThreads className="inline" />ds
      </div>
      ...
      <div className="w-11/12 max-w-sm bg-zinc-900/80 backdrop-blur-md rounded-2xl p-6 border border-zinc-800/50 shadow-2xl">
        ...
        <form onSubmit={onSubmit}>
          ...
          <button
            type="submit"
            className={`${buttonClass} ${isLogin ? 'bg-white text-zinc-950 hover:bg-zinc-200' : 'bg-green-500 text-white hover:bg-green-600'}`}
          >
